...
CatalogMeta::CatalogMeta(std::map<table_id_t, page_id_t> table_meta_pages_, std::map<index_id_t, page_id_t> index_meta_pages_)
{
...
CatalogMeta *CatalogMeta::DeserializeFrom(char *buf, MemHeap *heap) {
  ...
  auto catalogmeta = new (mem)CatalogMeta(table_meta_pages, index_meta_pages);
...
dberr_t CatalogManager::FlushCatalogMetaPage() const {
  ...
  /*
  // flush table meta pages
  auto table_meta_pages_it = catalog_meta_->table_meta_pages_.begin();
  for (; table_meta_pages_it != catalog_meta_->table_meta_pages_.end(); ++table_meta_pages_it)
  {
    bool b = buffer_pool_manager_->FlushPage(table_meta_pages_it->second);
    ASSERT(b, "Failed to flush table meta pages: pages pinned");
  }
  catalog_meta_->table_meta_pages_.clear();

  // flush index meta pages
  auto index_meta_pages_it = catalog_meta_->index_meta_pages_.begin();
  for (; index_meta_pages_it != catalog_meta_->index_meta_pages_.end(); ++index_meta_pages_it)
  {
    bool b = buffer_pool_manager_->FlushPage(index_meta_pages_it->second);
    ASSERT(b, "Failed to flush index meta pages: pages pinned");
  }
...
dberr_t CatalogManager::LoadTable(const table_id_t table_id, const page_id_t page_id) 
{
    ...
    if (table_meta != nullptr) 
    {
      ...
      auto *table_heap = TableHeap::Create(buffer_pool_manager_, table_meta->GetFirstPageId(), sch, log_manager_, lock_manager_, table_info->GetMemHeap());
...
dberr_t CatalogManager::LoadIndex(const index_id_t index_id, const page_id_t page_id) 
{
    ...
    if (index_meta != nullptr) 
    {
      ...
      index_names_[table_name] = index_name;
